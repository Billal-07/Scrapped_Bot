...
def extract_and_save_class_data(urls, class_name, output_filename):
    # Initialize a list to store extracted data
    data_list = []
    ...
    for url in urls:
        ...
        if response.status_code == 200:
            ...
            print(f"Data from class '{class_name}' for URL '{url}' extracted")
        else:
            print(f"Failed to retrieve the webpage. Status code: {response.status_code}")
...
# Read URLs from the "UI_UX.txt" file
with open("UI_UX.txt", "r") as file:
    urls = file.read().splitlines()

class_to_extract = "post-card"
output_file_name = "saved_data_all_urls.json"  # Use a single output file for all URLs
extract_and_save_class_data(urls, class_to_extract, output_file_name)
